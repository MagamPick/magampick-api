...
2. POST /api/v1/auth/signup
   { email, password, nickname, phone, verificationToken, agreedTermIds, address }
3. 서버: verificationToken 검증 → customers row + 약관 동의 + 기본 주소 생성 (phone_verified_at 기록)
...
2. POST /api/v1/auth/seller/signup
   ...
   - image: 첫 매장 대표 이미지 (선택)
3. 서버: 사업자 진위확인 + 지오코딩 + 이미지 업로드(이미지 전송 시)를 먼저 수행
...
- **셀프 가입 불가**.
- **부팅 시 시드**: 앱 시작 시 `admins` 테이블이 비어 있으면 `ADMIN_EMAIL` / `ADMIN_PASSWORD` / `ADMIN_NAME` 환경변수로 기본 admin 을 자동 생성 (`AdminSeedInitializer`).
  - 두 환경변수 중 하나라도 미설정이면 시드 건너뜀.
  - admin 이 이미 1명 이상이면 항상 건너뜀.
- 이후 추가 admin 생성 흐름 (초대 메일 등) 은 별도 구현 — 현재 미정.
...
|---|---|---|
| **만료** | **30분** | **14일** |
| **저장소** | 클라이언트만 (stateless) | HttpOnly 쿠키 + Redis 세션 |
...
- Redis 에 `refresh:{role}:{userId}:{tokenId}` 키로 토큰 hash 저장
- `Rotation` 없음 — 갱신 시 새 access 만 발급
- 동일 사용자가 여러 기기 로그인 시 refresh token id 별 세션 다중 보관
...
서버 처리:
1. JWT 서명 / 만료 검증
2. Redis 에서 refresh token hash 존재 / 유효 확인
3. 새 access 발급

응답: access 토큰. refresh 는 기존 HttpOnly 쿠키를 유지한다.
...
처리:
1. Redis 에서 **해당 기기의 refresh token 세션 삭제**
...
해당 사용자의 모든 refresh token 세션 삭제 → 다른 기기도 다음 갱신 시도 시 강제 로그아웃.
...
```
1. POST /api/v1/auth/password-resets/verify-identity { email, phone, verificationToken }
   → 이메일 계정 + 휴대폰 본인인증 토큰 확인 후 resetToken 발급 (Redis, 15분, 1회용)
2. POST /api/v1/auth/seller/password-resets/verify-identity { email, phone, verificationToken }
   → 사장 계정도 동일한 방식으로 resetToken 발급
3. POST /api/v1/auth/password-resets/confirm { resetToken, newPassword }
4. 비밀번호 변경 + 해당 사용자의 모든 refresh token 세션 무효화
5. 카카오 소셜 전용 계정(`password_hash = NULL`)은 `SOCIAL_ONLY_ACCOUNT` 로 거부
```
...
현재 비밀번호 확인 → 새 비밀번호 저장 → 현재 refresh 세션만 유지하고 나머지 세션 폐기
```
...
| `GET /api/v1/stores/**` (그 외 서브경로), `GET /api/v1/clearance-items/**` | ✅ | ✅ | ✅ | ✅ |
...
| `POST /auth/password-resets/verify-identity` | 3회 / 시간 / 이메일 | 출시 시점 |
...
| 로그인 실패 (비밀번호 불일치 / 이메일 없음) | `401` | `LOGIN_FAILED` |
| 권한 부족 | `403` | `FORBIDDEN` |
...
| 비밀번호 재설정 본인확인 실패 | `400` | `RESET_VERIFICATION_FAILED` |
| 현재 비밀번호 불일치 | `400` | `CURRENT_PASSWORD_MISMATCH` |
| 소셜 전용 계정의 비밀번호 사용 | `409` | `SOCIAL_ONLY_ACCOUNT` |
| 휴대폰 인증 미완료 | `400` | `PHONE_VERIFICATION_REQUIRED` |
패키지 기준:

- JWT / Spring Security 인프라 에러 (`TOKEN_EXPIRED`, `INVALID_TOKEN`, `REFRESH_INVALID`) 는 `global/security/exception/AuthErrorCode` 에 정의
- 회원가입 / 로그인 / 비밀번호 비즈니스 에러 (`LOGIN_FAILED`, `EMAIL_ALREADY_EXISTS`, `PHONE_VERIFICATION_REQUIRED`) 는 `auth/exception/AuthErrorCode` 에 정의
